...
**示例：**
	...
	void test01()
	{
		...
		//重新指定大小
		v1.resize(15, 100);     //利用重载版本（指的是resize的重载版本），可以指定默认填充值，参数2
		printVector(v1);  //如果指定的比原来长了，默认用0填充

		v1.resize(5);
		printVector(v1); //如果指定的比原来短了，超出部分会删除掉
	}
...
**示例：**
	...
	void test01()
	{
		...
		for (int i = 0; i < 100000; i++)
		{
			...
			if (p != &v1[0])   //如果p没有指向第一个元素的地址，就把它指向首地址，因为每开辟一次内存，首地址就会变化一次
			{
				p = &v1[0];
				num++;
			}
