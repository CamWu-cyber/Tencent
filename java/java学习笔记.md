...
### 函数式接口概述
函数式接口：有且仅有一个抽象方法的接口
Java 中的函数式接口 体现的就是 Lambda 表达式，所以函数式接口就是可以适用于 Lambda 使用的接口；
...
ComparatorDemo.java
        ...
        /*
            定义一个类（ComparatorDemo），在类中提供两个方法
                一个方法是：Coparator<String> getComparator()   方法返回值Comparator是一个函数式接口（官方的）
                一个方法是main方法，在main方法中调用getComparator方法
         */
        ...
        //        Collections.sort(array); // 按照字母顺序排序
                Collections.sort(array, getComparator());  // 按照自定义的顺序排序
...
* 前两节看的有点迷糊，疑问在于不清楚，为什么在<函数式接口作为方法的参数>中的函数式接口重写方法是在main函数中，而在<函数式接口作为方法的返回值>中的函数式接口重写方法不在main函中，个人当前的理解是，“重写方法”这件事是否发生，只跟是否新建该对象有关，新建接口对象的时候，才需要“重写方法”。是否正确按下不表，后续再看。
* 查看源码快捷键：选中方法后，ctrl+b
...
* T 泛型
* T get(): 获得结果
* 该方法不需要参数，它会按照某种实现逻辑 （由Lambda实现）返回一个数据
* Supplier<T> 接口也被称为**生产型接口**，如果我们指定了接口的泛型是什么类型，那么接口中的get方法就会产生什么类型的数据供我们使用

SupplierDemo.java

        package com_1.itheima_02;
